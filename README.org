...
change and the project's own row. Scoped by the cached series' own
project slug, so it also cleans up correctly even if the project was
queried directly and never got its own row in the local ~projects~
table.
